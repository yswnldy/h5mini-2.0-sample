package laya.webgl.canvas {
	...
	public class WebGLContext2D extends Context {
		...
		private static var _charBook:CharBook = new CharBook();
		...
		private function _fillText(txt:*, words:Vector.<HTMLChar>, x:Number, y:Number, fontStr:String, color:String, strokeColor:String, lineWidth:int, textAlign:String, underLine:int = 0):void {
			...
			*/
			if (txt)
				_charBook.filltext(this, txt, x, y, fontStr, color, strokeColor, lineWidth, textAlign, underLine);
			else if( words)
				_charBook.fillWords(this, words, x, y, fontStr, color, strokeColor, lineWidth);
		}
		
		public function _fast_filltext(data:WordText, x:Number, y:Number, fontObj:Object, color:int, strokeColor:int, lineWidth:int, textAlign:int, underLine:int = 0 ):void {
			_charBook._fast_filltext(this, data,null, x, y, fontObj as FontInfo, color, strokeColor, lineWidth, textAlign, underLine);
		}
		...
		private function _fillRect(x:Number, y:Number, width:Number, height:Number, rgba:uint):void {
			...
			var sameKey:Boolean = submit  && (submit._key.submitType === Submit.KEY_DRAWTEXTURE && submit._key.blendShader === _submitKey.blendShader);
			...
			if(!clipedOff(_transedPoints)){
				...
				if (!sameKey) {
					...
					copyClipInfo(submit, _globalClipMatrix);
		...
		public override function drawTextures(tex:Texture, pos:Array,tx:Number, ty:Number):void {
			...
			for (var i:int = 0; i < n; i++) {
				_inner_drawTexture(tex, tex.bitmap.id, pos[ipos++]+tx, pos[ipos++]+ty,0,0,null,null, 1.0);
			}
		...
		public function _drawTextureM(tex:Texture, x:Number, y:Number, width:Number, height:Number, m:Matrix, alpha:Number,uv:Array):Boolean{
			...
			return _inner_drawTexture(tex, tex.bitmap.id, x, y, width, height, m, uv, alpha);
		}
		...
		public function _drawRenderTexture(tex:RenderTexture2D, x:Number, y:Number, width:Number, height:Number, m:Matrix, alpha:Number, uv:Array):Boolean {
			return _inner_drawTexture(tex as Texture, -1, x, y, width, height, m, uv, 1.0);
		}
		...
		*/
		private function copyClipInfo(submit:Submit, clipInfo:Matrix):void {
		 ...
		 * @return
		 */
		public function _inner_drawTexture(tex:Texture, imgid:int, x:Number, y:Number, width:Number, height:Number, m:Matrix, uv:Array, alpha:Number):Boolean {
			...
			//var rgba = mixRGBandAlpha(_drawTextureUseColor?(fillStyle?fillStyle.toInt():0):0xffffffff); 	不支持白色文字调制颜色了
			//var rgba:int = mixRGBandAlpha(0xffffffff);
			//rgba = _mixRGBandAlpha(rgba, alpha);  这个连续调用有问题
			...
			transformQuad(x, y, width || tex.width, height || tex.height, _italicDeg, m || _curMat, _transedPoints);
		 ...
		 * @param	alpha
		 */
		override public function drawTextureWithTransform(tex:Texture, x:Number, y:Number, width:Number, height:Number, transform:Matrix, tx:Number, ty:Number, alpha:Number,blendMode:String):void {
			if (!transform) {
		...
		override public function drawCanvas(canvas:HTMLCanvas, x:Number, y:Number, width:Number, height:Number):void {
			...
			if (src._targets) {
				...
				*/
				...
				if ((canvas as Object).touches) {
					((canvas as Object).touches as Array).forEach(function(v:CharRenderInfo):void { v.touch(); } );
				}
		...
		public function drawTarget(rt:RenderTexture2D, x:Number, y:Number, width:Number, height:Number, m:Matrix, shaderValue:Value2D, uv:Array = null, blend:int = -1):Boolean {
			...
			if(!clipedOff(_transedPoints)){
				...
				copyClipInfo(submit as Submit, _globalClipMatrix);
				submit._numEle = 6;
				_mesh.indexNum += 6;
				_mesh.vertNum += 4;
		...
		override public function drawTriangles(tex:Texture, x:Number, y:Number, vertices:Float32Array, uvs:Float32Array, indices:Uint16Array, matrix:Matrix, alpha:Number, color:ColorFilter, blendMode:String):void {
			...
			var nBlend:int = BlendMode.TOINT[blendMode]||0;	//这个是在函数中控制，为了避免麻烦（恢复比较等）就直接使用这个参数，而不去修改contxt了
			
			var webGLImg:Bitmap = tex.bitmap as Bitmap;
			var preKey:SubmitKey = _curSubmit._key;
			var sameKey:Boolean = preKey.submitType === Submit.KEY_TRIANGLES && preKey.other === webGLImg.id && preKey.blendShader==nBlend;
			...
			if (!sameKey) {
				...
				submit.shaderValue.textureHost = tex;
				submit._renderType = Submit.TYPE_TEXTURE;
				submit._key.blendShader = nBlend;			//submitTexture使用的是context的，这里重载一下。
				submit._blendFn = _targets ? BlendMode.targetFns[nBlend] : BlendMode.fns[nBlend];
		...
		override public function flush():int {
			...
			_flushCnt++;
			//charbook gc
			if (_flushCnt % 600 == 0 && Render._context == this) {
		...
		public function fill():void {
			...
			var tPath:Path = _getPath();
			var submit:Submit = _curSubmit;
			var sameKey:Boolean = (submit._key.submitType === Submit.KEY_VG && submit._key.blendShader === _submitKey.blendShader);
			sameKey && (sameKey&&=isSameClipInfo(submit));
			if (!sameKey) {
