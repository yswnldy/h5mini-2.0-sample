package laya.webgl.resource { 
	...
	public class CharPages {
		...
		public function getWidth(str:String):Number {
			return charRender.getWidth(str);
		}
		 ...
		 * @return
		 */
		public function getChar(str:String, lineWidth:int, fontsize:int, color:uint, strokeColor:uint, bold:Boolean, touch:Boolean, scalekey:String):CharRenderInfo {
			if (str === ' ')
				return getSpaceChar(touch);
			var key:String = (lineWidth > 0?(str + '_'+lineWidth+strokeColor):str);
			var ret:CharRenderInfo;
			
			color = color & 0xffffff;//不要alpha
			...
			*/
			...
			var bmp:ImageData = getCharBmp(str, CharBook._curFont, lineWidth, colStr, strokeColStr, ret);
