package laya.d3.resource {
	...
	public class DataTexture2D extends BaseTexture {
		...
		//先只接收RGBA int8的
		public static function create(data:ArrayBuffer, w:int, h:int, magfilter:int =0x2601/*WebGLContext.LINEAR*/, minfilter:int =0x2601/*WebGLContext.LINEAR*/, mipmap:Boolean = true):DataTexture2D {
		 ...
		 */
		public static function load(url:String, w:int = 0, h:int = 0, magfilter:int =0x2601/*WebGLContext.LINEAR*/, minfilter:int =0x2601/*WebGLContext.LINEAR*/):DataTexture2D {
		...
		private function _createWebGlTexture():void {
			...
			if (this._mipmaps) {
				if (DataTexture2D.lodasatlas) {
					var infoi:int = 0;
					gl.texImage2D(_glTextureType, 0, WebGLContext.RGBA, _width, _height, 0, WebGLContext.RGBA, WebGLContext.UNSIGNED_BYTE, null);
					for (var i:int = 0; i < this._mipmaps.length; i++) {
						if (_mipmaps[i].byteLength != cw * ch * 4) {
							throw "mipmap size error  level:" + i;
						}
						gl.texSubImage2D(_glTextureType, 0, simLodRect[infoi++], simLodRect[infoi++], simLodRect[infoi++], simLodRect[infoi++], WebGLContext.RGBA, WebGLContext.UNSIGNED_BYTE, new Uint8Array(_mipmaps[i]));
					}
					...
					for (i = 0; i < this._mipmaps.length; i++) {
						...
						gl.texImage2D(_glTextureType, i, WebGLContext.RGBA, cw, ch, 0, WebGLContext.RGBA, WebGLContext.UNSIGNED_BYTE, new Uint8Array(_mipmaps[i]));
						cw /= 2;
						ch /= 2;
						if (cw < 1) cw = 1;
						if (ch < 1) ch = 1;
				...
				gl.texImage2D(_glTextureType, 0, WebGLContext.RGBA, w, h, 0, WebGLContext.RGBA, WebGLContext.UNSIGNED_BYTE, new Uint8Array(_buffer));
			}
			...
			//if (isPot) {
				...
				//
				//gl.texParameteri(_glTextureType, WebGLContext.TEXTURE_MIN_FILTER, minFifter);
				//gl.texParameteri(_glTextureType, WebGLContext.TEXTURE_MAG_FILTER, magFifter);
				//gl.texParameteri(_glTextureType, WebGLContext.TEXTURE_WRAP_S, repeat);
				//if (this._mipmaps)
					//gl.texParameteri(_glTextureType, WebGLContext.TEXTURE_WRAP_T, WebGLContext.CLAMP_TO_EDGE);
				//else
					//gl.texParameteri(_glTextureType, WebGLContext.TEXTURE_WRAP_T, repeat);
				//this._mipmap && gl.generateMipmap(_glTextureType);
			//} else {
			...
			//if (isPot)
				//_setGPUMemory(w * h * 4 * (1 + 1 / 3));//使用mipmap则在原来的基础上增加1/3
			//else
				//_setGPUMemory(w * h * 4);
			//_recreateLock = false;
		}
		 ...
		 *@private
		 */
		override public function _onAsynLoaded(data:*,propertyParams:Object=null):void {
