package laya.d3.core.scene {
	...
	public class Scene3D extends Sprite implements ISubmit, ICreateResource {
		 ...
		 * @param 天空渲染器。
		 */
		public function set skyRenderer(value:SkyRenderer):void {
			_skyRenderer = value;
		}
		...
		public function _clear(gl:WebGLContext, state:RenderContext3D):void {
			...
			if (clearFlag === BaseCamera.CLEARFLAG_SKY && !(camera.skyRenderer || _skyRenderer))
		...
		public function _renderScene(gl:WebGLContext, state:RenderContext3D, customShader:Shader3D = null):void {
			...
			if (camera.clearFlag === BaseCamera.CLEARFLAG_SKY) {
				var skyRenderer:SkyRenderer = camera.skyRenderer || _skyRenderer;
				if (skyRenderer)
					skyRenderer._render(state);
			}
		...
		override public function _parse(data:Object):void {
			...
			if (skyboxMaterialData) {
				_skyRenderer = new SkyRenderer();
				_skyRenderer.material = Loader.getRes(skyboxMaterialData.path);
			}
		...
		override public function destroy(destroyChild:Boolean = true):void {
			...
			(_skyRenderer) && (_skyRenderer.destroy());
			_lights = null;
			_lightmaps = null;
