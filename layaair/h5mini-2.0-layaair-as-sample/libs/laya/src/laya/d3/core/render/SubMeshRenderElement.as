		...
		override public function addToTransparentRenderQueue(context:RenderContext3D, queue:RenderQueue):void {
			...
			if (subMeshStaticBatch) {
				...
				if (staLastElement) {
					...
					if (staLastElement.staticBatch !== subMeshStaticBatch || staLastElement.material !== material || staLastRender.receiveShadow !== render.receiveShadow || staLastRender.lightmapIndex !== render.lightmapIndex) {
						elements.push(this);
				...
				if (_dynamicBatch) {
					...
					if (dynLastElement) {
						...
						if ((dynLastElement._geometry as SubMesh)._vertexBuffer._vertexDeclaration !== verDec || dynLastElement.material !== material || dynLastRender.receiveShadow !== render.receiveShadow || dynLastRender.lightmapIndex !== render.lightmapIndex) {
							elements.push(this);
							queue.lastTransparentBatched = false;
						} else {
							if (queue.lastTransparentBatched) {
