package laya.d3.core.scene {
	...
	public class OctreeNode implements ITreeNode {
		...
		public function OctreeNode(scene:Scene3D, currentDepth:int) {
			_scene = scene;
		...
		public function cullingObjects(boundFrustum:BoundFrustum, testVisible:Boolean, flags:int, cameraPosition:Vector3, projectionView:Matrix4x4):void {
			...
			//for (i = 0, n = _objects.length; i < n; i++) {
				...
				//if (Layer.isVisible(render._owner.layer.mask) && render.enable) {
					...
					//}
					//
					//render._renderUpdateWithCamera(projectionView);//TODO:静态合并或者动态合并造成浪费,多摄像机也会部分浪费
					...
					//for (j = 0, m = renderElements.length; j < m; j++) {
						...
						//var staticBatch:StaticBatch = renderElement.staticBatch;//TODO:换vertexBuffer后应该取消合并,修改顶点数据后，从动态列表移除，暂时忽略，不允许直接修改Buffer。
						//if (staticBatch/*(staticBatch._vertexDeclaration===renderElement.element.getVertexBuffer().vertexDeclaration)&&*/) {
							//staticBatch._addBatchRenderElement(renderElement);
						//} else {
							//var renderObj:IRenderable = renderElement._geometry;
							//if ((renderObj.triangleCount < DynamicBatch.maxCombineTriangleCount) && (renderObj._vertexBufferCount === 1) && (renderObj._getIndexBuffer()) && (renderElement.material.renderQueue < 2) && renderElement._canDynamicBatch && (!render._owner.isStatic))//TODO:是否可兼容无IB渲染,例如闪光//TODO:临时取消透明队列动态合并//TODO:加色法可以合并//TODO:静态物体如果没合并走动态合并现在会出BUG,lightmapUV问题。
								//dynamicBatchManager._addPrepareRenderElement(renderElement);
							//else
								//_scene._getRenderQueue(renderElement.material.renderQueue).addElement(renderElement);
						//}
					//}
				//}
				//
		 ...
		 * @private
		 */
		public function cullingShadowObjects(lightBoundFrustum:Vector.<BoundFrustum>, splitShadowQueues:Vector.<RenderQueue>, testVisible:Boolean, flags:int, scene:Scene3D):void {//TODO:SM
			////TODO:动态合并和静态合并
			//var i:int, j:int, n:int, m:int;
			//var dynamicBatchManager:DynamicBatchManager = _scene._dynamicBatchManager;
			//for (i = 0, n = _objects.length; i < n; i++) {
		 ...
		 * @private
		 */
		public function cullingShadowObjectsOnePSSM(lightBoundFrustum:BoundFrustum, splitShadowQueues:Vector.<RenderQueue>, lightViewProjectMatrix:Matrix4x4, testVisible:Boolean, flags:int, scene:Scene3D):void {//TODO:SM
			////TODO:动态合并和静态合并
