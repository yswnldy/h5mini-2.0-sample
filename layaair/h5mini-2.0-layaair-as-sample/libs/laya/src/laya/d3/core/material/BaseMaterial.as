package laya.d3.core.material {
	...
	public class BaseMaterial extends Resource implements IClone {
			...
			case "LAYAMATERIAL:02": 
				...
				for (var key:String in props) {
					...
					case "defines": 
						...
						for (i = 0, n = defineNames.length; i < n; i++) {
							var define:int = material._shader.getMaterialDefineByName(defineNames[i]);
							material._defineDatas.add(define);
						}
						break;
					case "renderStates": 
		 ...
		 */
		public function BaseMaterial(dataSize:int) {
			/*[DISABLE-ADD-VARIABLE-DEFAULT-VALUE]*/
			_defineDatas = new DefineDatas();
			_disablePublicDefineDatas = new DefineDatas();
			_shaderValues = new ShaderData(this, dataSize);
		...
		private function _removeTetxureReference():void {
			var values:Array = _shaderValues._data;
			for (var i:int = 0, n:int = values.length; i < n; i++) {//TODO:需要优化,杜绝is判断，慢
				var value:* = values[i];
		...
		override public function _addReference(count:int = 1):void {
			...
			var valueCount:int = _shaderValues._data.length;
			for (var i:int = 0, n:int = valueCount; i < n; i++) {//TODO:需要优化,杜绝is判断，慢
				var value:* = _shaderValues._data[i];
				if (value && value is BaseTexture)
		...
		public function setShaderName(name:String):void {
			...
			var passCount:int = _shader._passes.length;
