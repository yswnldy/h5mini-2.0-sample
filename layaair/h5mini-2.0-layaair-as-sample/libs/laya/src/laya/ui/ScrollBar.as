		...
		protected function loop():void {
			...
			if (_clickOnly) {
				if (Math.abs(_lastOffset * (isVertical ? Laya.stage._canvasTransform.getScaleY() : Laya.stage._canvasTransform.getScaleX())) > 1) {
					_clickOnly = false;
					if (this.value == 0&&_lastOffset>0)
					{
						if ((triggerDownDragLimit as Boolean) && triggerDownDragLimit())
						{
							cancelDragOp();
							return;
						}
					}
					if (this.value == max && _lastOffset < 0)
					{
						if ((triggerUpDragLimit as Boolean) && triggerUpDragLimit())
						{
							cancelDragOp();
							return;
						}
