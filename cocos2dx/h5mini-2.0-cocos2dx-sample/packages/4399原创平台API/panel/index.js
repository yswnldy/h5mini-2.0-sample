        ...
        window.build4399SDK = () => {
            ...
            if (FS.existsSync(localBuilderFilePath)) {
                ...
                if (!FS.existsSync(buildDir)) {
                    ...
                    this.$h3.innerHTML = '构建目录不存在,请重新构建项目:' + buildDir
                }
                else {
                    // 查找判断平台
                    if (data.platform === "web-mobile" || data.platform === "web-desktop") {
                        ...
                        if (FS.existsSync(indexHtmlFile)) {
                            ...
                            this.$h3.innerHTML = '添加SDK失败,文件不存在:' + indexHtmlFile
                        }
                        Editor.log("生成成功.");
                        this.$h3.innerHTML = '生成成功'
                    }
                    else {
                        Editor.log("SDK不支持该平台:" + data.platform);
                        this.$h3.innerHTML = 'SDK不支持该平台:' + data.platform
                    }
                }
            }
            else {
                Editor.log("请构建项目!");
                this.$h3.innerHTML = '请构建项目'
            }
        }
    },
