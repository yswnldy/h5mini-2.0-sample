...
当前案例使用了Cocos Creator v2.0.5版本进行开发。

此使用说明适用于**JavaScript、TypeScript**，示例项目使用的是JavaScript。

## API添加步骤

### 1.项目引入

> Cocos2dx支持直接使用原生js，有两种方式可引入h5api。

#### 1.1.通过扩展插件引入

##### 引入

> 在CocosCreator编辑器内选择`扩展/扩展商店`，弹出商店面板，找到`4399原创平台API`插件，单击后进图下载页，点击下载。

> 下载后需要自行解压，可以商店面板的右上角找到`下载管理`，找到`4399原创平台API`记录，点击`复制到文件夹`会启动解压程序，选择`{你的游戏项目根目录/packages}`目录。

你也可以直接把本示例项目的`{4399原创平台API}`文件夹复制到`{你的游戏项目根目录/packages}`目录下。

##### 使用

> 首先对你游戏的项目进行构建。

> 完成构建后，选择`扩展/plugin-4399-h5api`，将会打开`API插件面板`，选择`添加4399SDK`。

> 生成结果通过`API插件面板`即时提示，如生成失败请根据提示调整。

> `API插件面板`中的`添加4399SDK`按钮下方提示`生成成功`后即可在游戏中使用原创平台H5API。

> 此插件本质上只是执行标签引入步骤，在构建后的index.html模板里增加了原创平台H5API的地址。

##### 小提示

> 每次构建后只需重复`使用`的操作，推荐使用此方法。

#### 1.2.通过标签引入

##### 引入

> 请先进行构建，构建后在`{构建后的游戏项目根目录}`下的`index.html`页面中的<head></head>标签内加入以下代码：
...
> 无需其它操作即可在游戏中使用原创平台H5API。

##### 小提示

> 每次构建后都需要重复`引入`的操作，不推荐使用此方法。
...
> Cocos Creator扩展编辑器官方文档： https://docs.cocos.com/creator/manual/zh/extension/